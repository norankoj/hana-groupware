...
export default function ExcelUploadModal({
  ...
  // ── 공통 input 스타일 ─────────────────────────────────────────────────────
  const inputCls =
    "border border-line-strong rounded-lg px-2 py-1.5 text-xs outline-none focus:border-primary focus:ring-1 focus:ring-primary-soft bg-white";
  ...
  return (
        ...
        rows.length > 0 ? (
          <div className="flex items-center gap-3 w-full">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-primary text-white rounded-lg font-bold hover:bg-primary-active transition disabled:opacity-50 text-sm min-w-[100px]"
            >
              {saveProgress
    ...
    >
      <div className="p-6 space-y-5">
        {/* 안내 + 템플릿 다운로드 */}
        <div className={`${alertInfo} flex items-start justify-between gap-4`}>
          <div>
            <p className={`${alertTitle} ${alertInfoText} mb-1`}>사용 방법</p>
            <ol
              className={`${alertBody} ${alertInfoText} space-y-0.5 list-decimal list-inside`}
            >
          ...
          <button
            ...
            className="shrink-0 flex items-center gap-2 px-4 py-2.5 bg-white border border-primary-soft text-primary-active rounded-lg font-bold text-sm hover:bg-primary-soft transition whitespace-nowrap"
          >
        ...
        {/* 파일 업로드 영역 */}
        <div
          className="border-2 border-dashed border-line rounded-xl p-8 text-center cursor-pointer hover:border-primary hover:bg-primary-wash/30 transition-all"
        ...
        >
          ...
          {loading ? (
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-sm text-muted">파일 분석 중...</p>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-2">
              <svg className="w-10 h-10 text-disabled-text" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {rows.length > 0 && (
          <div className="overflow-x-auto rounded-xl border border-line">
            <table className={`${table} w-full`}>
              <thead>
                <tr>
                  <th className={`${th} text-left font-bold`}>상태</th>
                  <th className={`${th} text-left font-bold`}>날짜</th>
                  <th className={`${th} text-left font-bold`}>시간</th>
                  <th className={`${th} text-left font-bold`}>시설</th>
                  <th className={`${th} text-left font-bold`}>목적</th>
                  <th className={`${th} text-left font-bold`}>예약자</th>
                  <th className={`${th} text-center font-bold`}>수정</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((r, i) =>
                  editingIdx === i ? (
                      ...
                      <td colSpan={7} className={`${td}`}>
                        <div className="flex flex-wrap items-end gap-2">
                          <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-bold text-muted">날짜</label>
                            <input
                              type="date"
                              value={editDraft.날짜}
                          ...
                          <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-bold text-muted">시작시간</label>
                            <input
                              type="time"
                          ...
                          <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-bold text-muted">종료시간</label>
                            <input
                              type="time"
                          ...
                          <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-bold text-muted">시설</label>
                            <select
                              value={editDraft.시설명}
                          ...
                          <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-bold text-muted">목적</label>
                            <input
                              type="text"
                          ...
                          <div className="flex flex-col gap-1">
                            <label className="text-[10px] font-bold text-muted">예약자</label>
                            <input
                              type="text"
                              value={editDraft.예약자}
                          ...
                          <div className="flex gap-1.5 pb-[1px]">
                            <button
                              onClick={() => applyEdit(i)}
                              className="px-3 py-1.5 bg-primary text-white rounded-lg text-xs font-bold hover:bg-primary-active transition"
                            >
                              저장
                            </button>
                            <button
                    ...
                    <tr
                      key={i}
                      className={`border-b border-table-line ${r._error ? "bg-red-50" : "bg-white hover:bg-gray-50"}`}
                    >
                      <td className={`${td}`}>
                        {r._error ? (
                      ...
                      <td className={`${td}`}>{r.시설명}</td>
                      <td className={`${td} max-w-[160px] truncate`}>{r.사용목적}</td>
                      <td className={`${td}`}>{r.예약자}</td>
                      <td className={`${td} text-center`}>
                        <button
                          onClick={() => startEdit(i)}
                          title="수정"
                          className="inline-flex items-center justify-center w-6 h-6 rounded-md text-gray-400 hover:text-primary hover:bg-primary-wash transition"
                        >
                          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
