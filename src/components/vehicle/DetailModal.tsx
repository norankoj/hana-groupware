...
interface DetailModalProps {
  isOpen: boolean;
  onClose: () => void;
  selectedLog: VehicleLog | null; // 타입은 기존과 동일하게 유지
  currentUser: string | null;
  onRefresh: () => void; // ★ 데이터 업데이트 후 목록 새로고침용 함수
}
            ...
            <div className="space-y-4">
              ...
              {selectedLog.vehicle_status === "reserved" && (
                <div className="border-2 border-blue-100 bg-blue-50/50 p-5 rounded-xl">
                  ...
                  <label
                    className={`w-full flex items-center justify-center gap-2 bg-blue-600 text-white py-3.5 rounded-lg font-bold text-sm cursor-pointer transition shadow-md hover:bg-blue-700 active:scale-[0.98] ${
              ...
              {selectedLog.vehicle_status === "in_use" && (
                <div className="border-2 border-green-100 bg-green-50/50 p-5 rounded-xl">
                  ...
                  <label
                    className={`w-full flex items-center justify-center gap-2 bg-green-600 text-white py-3.5 rounded-lg font-bold text-sm cursor-pointer transition shadow-md hover:bg-green-700 active:scale-[0.98] ${
                      uploading ? "opacity-70 cursor-wait" : ""
                    }`}
                  >
                    {uploading ? (
