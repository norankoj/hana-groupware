...
function AlarmDetailPopup({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase
        .from("reservations")
        .select("id, start_at, end_at, purpose, driver_name, department, destination, profiles:user_id(full_name), resources:resource_id(name, category)")
  ...
  const skeletons = (
    <div className="space-y-3">
      {[1, 2, 3].map((i) => <div key={i} className="animate-pulse bg-gray-100 rounded-xl h-[100px]" />)}
    </div>
  );
  ...
  // 카드 공통 — 고정 높이 100px
  const cardCls = "bg-white border border-gray-200 rounded-xl p-4 shadow-sm cursor-pointer hover:border-gray-300 hover:shadow-md transition-all flex flex-col justify-between h-[100px]";

  const facilityCards = loading ? skeletons : (data?.facility.length ?? 0) === 0 ? empty("오늘 시설 예약 없음") : (
    <div className="space-y-3">
      {data!.facility.map((r) => {
        const start = new Date(r.start_at); const end = new Date(r.end_at);
        return (
          <div key={r.id} className={cardCls} onClick={(e) => { const rect = (e.currentTarget as HTMLElement).getBoundingClientRect(); setPopover({ reservation: r, isVehicle: false, rect }); }}>
            <div className="flex items-center justify-between gap-2">
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-purple-50 text-purple-700 truncate max-w-[120px]">{r.resources?.name ?? "시설"}</span>
              <span className="text-[10px] text-gray-400 tabular-nums shrink-0">{format(start, "HH:mm")}~{format(end, "HH:mm")}</span>
            </div>
            <p className="text-sm font-semibold text-gray-800 leading-snug line-clamp-1">{r.purpose || "예약"}</p>
            <p className="text-xs text-gray-400">예약자 <span className="text-gray-600 font-medium">{r.profiles?.full_name ?? "—"}</span></p>
          </div>
        );
      })}
    </div>
  ...
  const vehicleCards = loading ? skeletons : (data?.vehicle.length ?? 0) === 0 ? empty("오늘 차량 예약 없음") : (
    <div className="space-y-3">
      {data!.vehicle.map((r) => {
        const start = new Date(r.start_at); const end = new Date(r.end_at);
        return (
          <div key={r.id} className={cardCls} onClick={(e) => { const rect = (e.currentTarget as HTMLElement).getBoundingClientRect(); setPopover({ reservation: r, isVehicle: true, rect }); }}>
            <div className="flex items-center justify-between gap-2">
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-green-50 text-green-700 truncate max-w-[120px]">{r.resources?.name ?? "차량"}</span>
              <span className="text-[10px] text-gray-400 tabular-nums shrink-0">{format(start, "HH:mm")}~{format(end, "HH:mm")}</span>
            </div>
            <p className="text-sm font-semibold text-gray-800 leading-snug line-clamp-1">{r.purpose || "운행"}</p>
            <p className="text-xs text-gray-400">
              운전자 <span className="text-gray-600 font-medium">{r.driver_name ?? "—"}</span>
              {r.department && <span className="text-gray-400"> · {r.department}</span>}
            </p>
          </div>
        );
      })}
  ...
  const noticeCards = loading ? skeletons : (data?.notices.length ?? 0) === 0 ? empty("공지 없음") : (
    <div className="space-y-3">
      {data!.notices.map((n) => (
        <div key={n.id} className={cardCls} onClick={(e) => { const rect = (e.currentTarget as HTMLElement).getBoundingClientRect(); setPopover({ notice: n, rect }); }}>
          <div className="flex items-center justify-between gap-2">
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-600">{n.category}</span>
            <span className="text-[10px] text-gray-400 shrink-0">{format(new Date(n.created_at), "M.d (EEE)", { locale: ko })}</span>
          </div>
          <p className="text-sm font-semibold text-gray-800 leading-snug line-clamp-1">{n.title}</p>
  ...
  const colHeader = (label: string, count: number, badgeClass: string) => (
    <div className="flex items-center gap-2 mb-3 shrink-0">
      ...
      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${badgeClass}`}>{count}</span>
    </div>
  );
  ...
  const headerBg  = isVehicle ? "bg-green-50" : n ? "bg-blue-50" : "bg-purple-50";
  const labelCls  = isVehicle ? "text-green-600" : n ? "text-blue-600" : "text-purple-600";
        ...
        <div className="flex-1 overflow-hidden">
          ...
          <div className="flex h-full divide-x divide-gray-100 overflow-x-auto">
            <div className="flex-1 min-w-[300px] flex flex-col px-6 py-6 overflow-hidden">
              {colHeader("시설 예약", data?.facility.length ?? 0, "bg-purple-50 text-purple-700")}
              <div className="flex-1 overflow-y-auto pr-1.5" style={{ scrollbarWidth: "thin", scrollbarColor: "#d1d5db transparent" }}>{facilityCards}</div>
            </div>
            <div className="flex-1 min-w-[300px] flex flex-col px-6 py-6 overflow-hidden">
              {colHeader("차량 예약", data?.vehicle.length ?? 0, "bg-green-50 text-green-700")}
              <div className="flex-1 overflow-y-auto pr-1.5" style={{ scrollbarWidth: "thin", scrollbarColor: "#d1d5db transparent" }}>{vehicleCards}</div>
            </div>
            <div className="flex-1 min-w-[300px] flex flex-col px-6 py-6 overflow-hidden">
              {colHeader(
                "최근 공지",
              ...
              <div className="flex-1 overflow-y-auto pr-1.5" style={{ scrollbarWidth: "thin", scrollbarColor: "#d1d5db transparent" }}>{noticeCards}</div>
            </div>
          </div>
        </div>
      </Modal>
      ...
      {popover && createPortal(
        <>
          ...
          <div
            ...
            style={{
              top: Math.max(16, Math.min(popover.rect.top, window.innerHeight - 380)),
              left: popover.rect.right + 354 < window.innerWidth
          ...
          >
            ...
            <div className={`px-5 py-4 flex items-start justify-between gap-3 ${headerBg}`}>
              <div className="min-w-0">
                <span className={`text-xs font-bold ${labelCls}`}>{typeLabel}</span>
            ...
            <div className="px-5 py-4 space-y-3">
              {r && (
                <>
                  ...
                  />
                  {isVehicle && r.destination && <InfoRow label="목적지" value={r.destination} />}
                  {r.purpose && <InfoRow label={isVehicle ? "운행 목적" : "사용 목적"} value={r.purpose} />}
                  ...
                  {isVehicle ? (
                    <>
                      ...
                      {r.profiles?.full_name && (
                        <InfoRow label="예약자" value={r.profiles.full_name} />
                      )}
                    </>
                  ) : (
                    <InfoRow label="예약자" value={`${r.profiles?.full_name ?? "—"}`} />
              ...
              {n && (
                <>
                  <InfoRow
                    label="등록일"
                    value={format(new Date(n.created_at), "yyyy년 M월 d일 (EEE)", { locale: ko })}
                  />
                  <div className="border-t border-gray-100 pt-1" />
                  <Link
                    href="/notice"
      ...
      <div className="lg:w-2/5 min-w-0 bg-white rounded-2xl border border-gray-200 p-5 flex flex-col h-[260px] sm:h-[168px] overflow-hidden">
        <div className="flex items-center justify-between mb-2 shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1 text-[11px] font-bold text-blue-500 hover:text-blue-700 hover:bg-blue-50 px-2 py-0.5 rounded-md transition"
          >
            오늘 현황
            <svg
              className="w-3 h-3"
