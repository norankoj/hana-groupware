...
export default function CalendarSection({
  ...
  // xl(1280px+) → 기본 열림 / lg(노트북) → 기본 접힘
  const [isPanelOpen, setIsPanelOpen] = useState(false);
  useEffect(() => {
    setIsPanelOpen(window.innerWidth >= 1280);
  }, []);
  ...
  return (
    <>
      ...
      <section className="flex-1 order-2 2xl:order-1 bg-white rounded-xl border border-gray-200 overflow-hidden flex flex-col h-auto lg:h-[780px]">
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 flex items-center justify-between bg-gray-50/30 shrink-0 gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            <button
              ...
              title={isPanelOpen ? "일정 패널 접기" : "선택 날짜 일정 보기"}
              className="hidden lg:flex items-center gap-1 px-2.5 py-1.5 rounded-md border border-gray-200 text-xs text-gray-500 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 transition whitespace-nowrap"
            >
              <svg
              ...
              >
                <path
                  ...
                  d={
                    isPanelOpen
                      ? "M13 5l7 7-7 7M5 5l7 7-7 7" // 닫기 (»)
                      : "M11 19l-7-7 7-7m8 14l-7-7 7-7" // 열기 («)
                  }
                />
              </svg>
              {isPanelOpen ? "접기" : "일정 보기"}
            </button>
            <button
        ...
        <div className="flex flex-col lg:flex-row flex-1 overflow-visible lg:overflow-hidden">
          ...
          {/* 사이드 패널 (상세 목록) — lg 이상에서 toggle, 모바일은 항상 표시 */}
          <div
            className={`w-full lg:w-44 xl:w-80 bg-white flex flex-col h-auto lg:h-full lg:border-l border-t lg:border-t-0 border-gray-200 ${isPanelOpen ? "lg:flex" : "lg:hidden"}`}
          >
            <div className="px-3 xl:px-5 py-3 xl:py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50 xl:h-[72px] shrink-0">
              <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between w-full gap-0.5">
                <h4 className="text-xs xl:text-sm font-bold text-gray-800 flex items-center gap-1.5 truncate">
                ...
                <span className="text-[10px] xl:text-xs font-medium text-gray-500 pl-3 xl:bg-white xl:px-2 xl:py-0.5 xl:rounded xl:border xl:border-gray-200">
                ...
                <ul className="divide-y divide-gray-100">
                  {selectedEvents.map((e) => (
                    <li
                      ...
                      className="group flex flex-col xl:flex-row xl:items-center xl:justify-between px-3 xl:px-5 py-2.5 xl:py-3.5 hover:bg-gray-50 transition-colors cursor-pointer gap-1 xl:gap-0"
                    >
                      {/* lg: 세로 / xl: 기존 가로 레이아웃 */}
                      <div className="flex items-center gap-1.5 xl:gap-3 overflow-hidden">
                        <div
                          className={`flex-shrink-0 w-5 h-5 xl:w-9 xl:h-9 rounded-full flex items-center justify-center font-bold text-[9px] xl:text-xs border ${e.type === "schedule" ? "bg-teal-50 border-teal-200 text-teal-600" : "bg-gray-100 border-gray-200 text-gray-500"}`}
                        >
                          {e.profiles.full_name.slice(0, 1)}
                        </div>
                        <div className="flex flex-col min-w-0">
                          <span className="text-xs xl:text-sm font-bold text-gray-900 truncate">
                            {e.display_name}
                          </span>
                          <span
                            className={`text-[10px] xl:text-xs truncate mt-0.5 ${e.type === "schedule" ? "font-semibold text-teal-600" : "text-gray-500"}`}
                          >
                            {e.title} {e.location && `(${e.location})`}
                          </span>
                        </div>
                      </div>
                      <div className="flex xl:flex-col items-center xl:items-end gap-1.5 xl:gap-1 flex-shrink-0 pl-6 xl:pl-2">
                        <span
                          className={`px-1.5 py-0.5 rounded text-[9px] xl:text-[10px] font-bold border ${e.type === "schedule" ? "bg-teal-50 text-teal-600 border-teal-100" : "bg-blue-50 text-blue-600 border-blue-100"}`}
                        >
                          {e.type === "schedule" ? "일정" : "휴가"}
                        </span>
                        <span className="text-[9px] xl:text-[11px] text-gray-400 font-medium tabular-nums">
