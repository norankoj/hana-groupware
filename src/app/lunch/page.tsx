...
const RouletteGame = dynamic(() => import("@/components/lunch/RouletteGame"), { ssr: false });
const LotteryGame = dynamic(() => import("@/components/lunch/LotteryGame"), { ssr: false });
const LadderGame = dynamic(() => import("@/components/lunch/LadderGame"), { ssr: false });
...
export default function LunchPage() {
  ...
  const startSpin = () => {
    ...
    intervalRef.current = setInterval(() => {
      setDisplayMenu(restaurants[Math.floor(Math.random() * restaurants.length)]);
    }, 50);
  ...
  const fireConfetti = () => {
    ...
    const iv: any = setInterval(() => {
      ...
      confetti({ startVelocity: 30, spread: 360, ticks: 60, zIndex: 0, particleCount: cnt, origin: { x: rand(0.1, 0.3), y: Math.random() - 0.2 } });
      confetti({ startVelocity: 30, spread: 360, ticks: 60, zIndex: 0, particleCount: cnt, origin: { x: rand(0.7, 0.9), y: Math.random() - 0.2 } });
    }, 250);
  };
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center h-screen flex-col gap-4">
        ...
        <span className="text-xl font-bold text-indigo-600">재미로만 봐주세요...</span>
      </div>
    );
  ...
  return (
    <div className="max-w-xl mx-auto py-8 px-4 flex flex-col items-center min-h-screen pb-32">
      ...
      <div className="text-center mb-6">
        <h1 className="text-3xl font-extrabold text-gray-900 mb-1">🍽️ 오늘 점심 뭐먹지?!🤔</h1>
      ...
      {tab === "restaurant" && (
        <div className="w-full flex flex-col items-center gap-0">
          <p className="text-gray-500 text-sm mb-6 text-center">
            ...
            현재 <span className="font-bold text-gray-800">'{currentKeyword}'</span>(으)로 로딩됨
            <br />
            <button onClick={() => { setHistory([]); fetchLunch(); }} className="mt-1 text-xs text-gray-400 underline hover:text-gray-600">
              다른 종류 메뉴로 다시 찾기 ↻
            </button>
          </p>
          ...
          <div className="w-full bg-white rounded-3xl shadow-xl border-4 border-indigo-500 p-8 text-center relative overflow-hidden min-h-[320px] flex flex-col justify-center items-center">
            ...
            {displayMenu && (
              <div className="mb-4 animate-fadeIn relative z-10">
                <div className="text-6xl mb-4 animate-bounce">{displayMenu.icon}</div>
                <div className={`text-3xl font-black text-gray-800 break-keep leading-tight transition-all ${isSpinning ? "blur-sm scale-95 opacity-70" : "scale-100"}`}>
                  {displayMenu.name}
                </div>
                <div className="mt-3 flex gap-2 justify-center">
                  <span className="text-indigo-500 font-bold text-xs bg-indigo-50 px-2 py-1 rounded-full">#{displayMenu.category}</span>
                  <span className="text-gray-500 font-bold text-xs bg-gray-100 px-2 py-1 rounded-full">{displayMenu.address}</span>
                </div>
                <div className="mt-1 text-gray-400 text-xs">{displayMenu.phone || "번호 정보 없음"}</div>
              </div>
            )}
            {!isSpinning && displayMenu && (
              <a
                href={displayMenu.url} target="_blank" rel="noopener noreferrer"
          ...
          {history.length > 0 && (
            <div className="w-full mt-10">
              <h3 className="text-sm font-bold text-gray-500 mb-3 ml-1">오늘의 당첨 기록</h3>
                      ...
                      >
                        <span className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col text-left">
                            <span className="font-medium text-gray-700 group-hover:text-indigo-600">{menu.name}</span>
                            <span className="text-[10px] text-gray-400">{menu.category} · {menu.address}</span>
                          </div>
                        </span>
                        <span className="text-xs text-indigo-400 font-bold">이동 ↗</span>
                      </a>
                    </li>
                  ))}
                </ul>
      ...
      {tab !== "restaurant" && (
        <div className="w-full flex flex-col gap-6">
          ...
          <div className="w-full bg-white rounded-2xl border border-gray-200 p-4">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">참가자 이름</p>

            {/* 기존 이름 목록 */}
            <div className="flex flex-wrap gap-2 mb-3">
              {names.map((name, i) => (
                <div key={i} className="flex items-center gap-1 bg-indigo-50 border border-indigo-200 rounded-full px-3 py-1">
                  <input
                    type="text"
                    value={name}
            ...
            {validNames.length > 0 && (
              <p className="text-xs text-gray-400 mt-2">
                총 <span className="font-bold text-indigo-600">{validNames.length}명</span> 참가
              </p>
            )}
          </div>
